...
### Random Effect Model Marker Estimates -----------------------
ab = read.table("GBlupNoHerdModel/RnSoln_animal.dat",header=F,skip=1); ab= ab$V4
ab_ebv = tail(ab,n=as.numeric(args[1]))

### Load Truth Data Results ------------------------------------

true_data = read.table("PedigreeASV.txt",header=T); true_data = tail(true_data,as.numeric(args[1]))
...
# ### Fixed Model ---------------------------------------------
# 
# print(paste("Fixed Model : Accuracy of AB EBVs is",round(cor(ab_febv,tgv),digits=3),sep=" ")) ; plot(ab_febv,tgv,main="AlphaBayes EBV vs TBV : Fixed Model",pch=16)
...
#print(paste("Random Model : Accuracy of WOMBAT EBVs is",round(cor(ab_rebv,tgv),digits=3),sep=" ")) ; plot(ab_rebv,tgv,main="WOMBAT EBV vs TBV : Random Model",pch=16)
...
print(paste("No Herd Model : Accuracy of WOMBAT GEBVs is",round(cor(ab_ebv,tgv),digits=3),sep=" ")) ; plot(ab_ebv,tgv,main="WOMBAT GEBV vs TBV : No Herd Model",pch=16)
abline(lm(tgv~ab_ebv),lty=2,col="limegreen")

acc = round(cor(ab_ebv,tgv),digits=3)
bv_reg= lm(tgv~ab_ebv)
bv_bias = bv_reg$coefficients[2]-1
bv_int = bv_reg$coefficients[1]

#summary(tgv)
max_tgv_index = which(tgv>0.229)
max_tgv = tgv[max_tgv_index]
max_ebv = ab_ebv[max_tgv_index]
UQ_cor = cor(max_tgv,max_ebv)

min_tgv_index = which(tgv>-0.242)
min_tgv = tgv[min_tgv_index]
min_ebv = ab_ebv[min_tgv_index]
LQ_cor = cor(min_tgv,min_ebv)
